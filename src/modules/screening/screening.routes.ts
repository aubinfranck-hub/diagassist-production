...
export function registerScreening(app:Express, server:Server, deps:{requireAuth:any; getEffectivePlan:(phone:string)=>string; sessions:Map<string, any>}) {
  ...
  app.post("/api/screening/sessions", deps.requireAuth, (req:any,res)=>{
    ...
    const s={id,technicianPhone:req.session.phone,pairingCode:code(),status:"pending",createdAt:now,expiresAt:now+SESSION_TTL,frameCount:0};
    sessions.set(id,s);
  ...
  wss.on("connection",(ws:any)=>{
    ...
    ws.on("message",(raw:Buffer)=>{ try {
      ...
      if(m.type==="frame"&&role==="technician"){ const s=getSession(sid); if(s){s.frameCount++; sendAll(sid,m,ws);} }
      else if(m.type==="command"&&role==="coach"){ if(!ALLOWED.has(m.payload?.action)) return ws.send(JSON.stringify({type:"error",message:"Commande non autorisée."})); sendAll(sid,m,ws); }
