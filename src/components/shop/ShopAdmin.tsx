...
function ProductForm({ auth, categories, onDone, onCancel }: { auth: any; categories: any[]; onDone: () => void; onCancel: () => void }) {
  const [name, setName] = useState(""), [brand, setBrand] = useState(""), [model, setModel] = useState(""), [categoryId, setCategoryId] = useState(""), [priceEur, setPriceEur] = useState(""), [description, setDescription] = useState(""), [specs, setSpecs] = useState(""), [compatibility, setCompatibility] = useState(""), [boxContents, setBoxContents] = useState(""), [warranty, setWarranty] = useState("");
  const scannerReferences = [
    { brand: "AUTEL", model: "MaxiCOM MK808BT Pro", name: "AUTEL MaxiCOM MK808BT Pro", photo: "https://store.autel.com/cdn/shop/files/MK808BT_Pro_01_600x.png?v=1700467619" },
    { brand: "AUTEL", model: "MaxiCOM MK900-BT", name: "AUTEL MaxiCOM MK900-BT", photo: "https://store.autel.com/cdn/shop/files/MK900-BT_03_1_600x.png?v=1732093424" },
    { brand: "AUTEL", model: "MaxiSYS MS906 MAX", name: "AUTEL MaxiSYS MS906 MAX" },
    { brand: "AUTEL", model: "MaxiSYS Ultra S2", name: "AUTEL MaxiSYS Ultra S2" },
    { brand: "XTOOL", model: "D7", name: "XTOOL D7" },
    { brand: "XTOOL", model: "D7X", name: "XTOOL D7X" },
    { brand: "XTOOL", model: "D7W", name: "XTOOL D7W" },
    { brand: "XTOOL", model: "D8S", name: "XTOOL D8S" },
    { brand: "XTOOL", model: "D8W", name: "XTOOL D8W" },
    { brand: "XTOOL", model: "D9S", name: "XTOOL D9S" },
    { brand: "XTOOL", model: "D9S Pro", name: "XTOOL D9S Pro" },
    { brand: "XTOOL", model: "D9 MAX", name: "XTOOL D9 MAX" },
    { brand: "XTOOL", model: "IP900BT", name: "XTOOL IP900BT" },
    { brand: "XTOOL", model: "IP919 MAX", name: "XTOOL IP919 MAX" },
    { brand: "THINKCAR", model: "ThinkDiag 2", name: "THINKCAR ThinkDiag 2" },
    { brand: "HUMZOR", model: "Z100 Elite", name: "HUMZOR Z100 Elite" },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!brand.trim() || !model.trim()) { alert("Pour un scanner, la marque et le modèle exact sont obligatoires."); return; }
    if (photos.length === 0) { alert("Ajoutez au moins une photo réelle du modèle avant de créer la fiche."); return; }
    setSaving(true);
    await shopFetch(auth, "/api/admin/shop/products", { method: "POST", body: JSON.stringify({ name, brand, model, category_id: categoryId || null, price_eur: priceEur ? Number(priceEur) : null, description, specs, compatibility, box_contents: boxContents, warranty, photos, videos }) });
    setSaving(false); onDone();
  };
  return <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2.5"><div className="flex justify-between items-center mb-1"><p className="text-sm font-bold text-white">Nouveau produit</p><button type="button" onClick={onCancel} className="text-slate-500 cursor-pointer"><X className="w-4 h-4" /></button></div><div className="space-y-2">
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><input required placeholder="Nom du produit" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" /><input placeholder="Marque (AUTEL, XTOOL...)" value={brand} onChange={(e) => setBrand(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" /><input placeholder="Modèle / référence exacte" value={model} onChange={(e) => setModel(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" /></div>
    <div className="rounded-xl bg-slate-950 border border-slate-800 p-3">
      <p className="text-[10px] uppercase tracking-wide text-slate-500 mb-2">Références scanners à ajouter</p>
      <div className="flex flex-wrap gap-1.5">{scannerReferences.map((r) => <button type="button" key={r.model} onClick={() => { setBrand(r.brand); setModel(r.model); setName(r.name); setPhotos(r.photo ? [r.photo] : []); }} className="px-2.5 py-1.5 rounded-lg border border-slate-700 bg-slate-900 hover:border-red-500 text-[10px] font-semibold text-slate-300 hover:text-white">{r.name}</button>)}</div>
      <p className="text-[10px] text-amber-400 mt-2">Les modèles avec photo officielle vérifiée sont préremplis automatiquement. Pour les autres, ajoutez la photo réelle du modèle avant création.</p>
    </div>
  </div><div className="flex gap-2"><select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="flex-1 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white"><option value="">Catégorie...</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select><div className="w-44"><input type="number" min="0" step="0.01" placeholder="Prix fournisseur (€)" value={priceEur} onChange={(e) => setPriceEur(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" />{priceEur && <p className="text-[10px] text-emerald-400 mt-1">{(Math.ceil((Number(priceEur) * 655.957) / 1000) * 1000).toLocaleString("fr-FR")} FCFA</p>}</div></div><textarea placeholder="Description" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white resize-none" /><textarea placeholder="Caractéristiques" rows={2} value={specs} onChange={(e) => setSpecs(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white resize-none" /><textarea placeholder="Compatibilité" rows={2} value={compatibility} onChange={(e) => setCompatibility(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white resize-none" /><input placeholder="Contenu de la boîte" value={boxContents} onChange={(e) => setBoxContents(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" /><input placeholder="Garantie" value={warranty} onChange={(e) => setWarranty(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" /><div><label className="text-xs text-slate-400">Photos du modèle exact ({photos.length})</label><p className="text-[10px] text-amber-400 mt-1">Photo dédiée au modèle : ne pas utiliser l’image d’un autre scanner.</p><input type="file" accept="image/*" multiple onChange={(e) => (Array.from(e.target.files || []) as File[]).forEach((f) => addFile(f, setPhotos))} className="block w-full text-xs text-slate-400 mt-1" /></div><div><label className="text-xs text-slate-400">Vidéos ({videos.length})</label><input type="file" accept="video/*" multiple onChange={(e) => (Array.from(e.target.files || []) as File[]).forEach((f) => addFile(f, setVideos))} className="block w-full text-xs text-slate-400 mt-1" /></div><button type="submit" disabled={saving} className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-bold py-2.5 rounded-lg cursor-pointer">{saving ? "Enregistrement..." : "Créer le produit"}</button></form>;
}
