...
function ProductForm({ auth, categories, onDone, onCancel }: { auth: any; categories: any[]; onDone: () => void; onCancel: () => void }) {
  const [name, setName] = useState(""), [brand, setBrand] = useState(""), [model, setModel] = useState(""), [categoryId, setCategoryId] = useState(""), [price, setPrice] = useState(""), [description, setDescription] = useState(""), [specs, setSpecs] = useState(""), [compatibility, setCompatibility] = useState(""), [boxContents, setBoxContents] = useState(""), [warranty, setWarranty] = useState("");
