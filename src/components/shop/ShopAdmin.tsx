...
function ProductForm({ auth, categories, onDone, onCancel }: { auth: any; categories: any[]; onDone: () => void; onCancel: () => void }) {
  ...
  const scannerReferences = [
    { brand: "THINKCAR", model: "ThinkDiag 2", name: "THINKCAR ThinkDiag 2", photo: "https://mythinkcarfr.com/cdn/shop/files/1_f0aa069e-bd83-45f6-a74c-23fde18caeee_1170x.jpg?v=1741769032" },
    { brand: "KINGBOLEN", model: "EDIAG ELITE", name: "KINGBOLEN EDIAG ELITE", photo: "https://i.ebayimg.com/images/g/5dEAAOSwOLdoC0qq/s-l1200.jpg" },
    { brand: "TOPDON", model: "TopScan Pro", name: "TOPDON TopScan Pro", photo: "https://cdn.pacifiko.com/image/cache/catalog/p/MGQ3ODk5ZG-500x500.jpg" },
    { brand: "HUMZOR", model: "Z100 Elite", name: "HUMZOR Z100 Elite", photo: "https://www.vxdas.com/cdn/shop/files/HUMZORZ100Elite2.jpg?v=1739600069" },
    { brand: "LAUNCH", model: "X431 Elite", name: "LAUNCH X431 Elite", photo: "https://www.truckdiagnosticscanner.com/photo/ps171947783-launch_x431_elite_car_full_system_diagnostic_tools_auto_obd_obd2_scanner_active_test_ecu_coding_multilingual_free_update.jpg" },
  ];
  ...
  return <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2.5"><div className="flex justify-between items-center mb-1"><p className="text-sm font-bold text-white">Nouveau produit</p><button type="button" onClick={onCancel} className="text-slate-500 cursor-pointer"><X className="w-4 h-4" /></button></div><div className="space-y-2">
    ...
    <div className="rounded-xl bg-slate-950 border border-slate-800 p-3">
      ...
      <div className="flex flex-wrap gap-1.5">{scannerReferences.map((r) => <button type="button" key={r.model} onClick={() => { setBrand(r.brand); setModel(r.model); setName(r.name); setPhotos([r.photo]); }} className="px-2.5 py-1.5 rounded-lg border border-slate-700 bg-slate-900 hover:border-red-500 text-[10px] font-semibold text-slate-300 hover:text-white">{r.name}</button>)}</div>
      <p className="text-[10px] text-amber-400 mt-2">La sélection préremplit uniquement la marque et le modèle. La photo du modèle exact reste obligatoire.</p>
    </div>
